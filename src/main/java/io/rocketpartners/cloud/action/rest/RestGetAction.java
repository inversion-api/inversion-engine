...
public abstract class RestGetAction<T extends RestGetAction> extends Action<T>
{
   ...
   protected abstract QueryResults runQuery(Service service, Api api, Endpoint endpoint, Chain chain, Request req, Response res) throws Exception;
   ...
   public void run(Service service, Api api, Endpoint endpoint, Chain chain, Request req, Response res) throws Exception
   {
      if (req.getCollection() != null && req.getEntityKey() != null)
      {
         ...
         {
            //todo, add support for comma separated keys

            List<Term> terms = req.getCollection().getEntity().decodeKey(req.getEntityKey());

            for (Term term : terms)
            {
               req.getUrl().withParam(term.getToken(0), term.getToken(1));
            }
         }
         else
         {
      ...
      QueryResults results = runQuery(service, api, endpoint, chain, req, res);
      ...
      else
      {
         ...
         for (ObjectNode node : results.getRows())
         {
            if (collection != null)
            {
               ...
               if (Utils.empty(href))
               {
                  String entityKey = req.getCollection().getEntity().encodeKey(node);
   ...
   public T withMaxRows(int maxRows)
   {
