...
import io.inversion.cloud.action.elastic.v03x.rql.Parser;
import io.inversion.cloud.action.elastic.v03x.rql.Predicate;
import io.inversion.cloud.action.elastic.v03x.rql.Rql;
import io.inversion.cloud.action.elastic.v03x.rql.Stmt;
...
public class ElasticRql extends Rql
{
   ...
   public QueryDsl toQueryDsl(Map<String, String> params) throws Exception
   {
      ...
      List<io.inversion.cloud.action.elastic.v03x.rql.Order> orderList = stmt.order;
      //      if (orderList.size() > 0)
      //      {
      boolean idSortExists = false;
      for (io.inversion.cloud.action.elastic.v03x.rql.Order order : orderList)
      {
         if (Parser.dequote(order.col).equalsIgnoreCase("id"))
            idSortExists = true;
      }
      if (!idSortExists)
         orderList.add(new io.inversion.cloud.action.elastic.v03x.rql.Order("id", "asc"));
      //      }
      for (int i = 0; i < orderList.size(); i++)
      {
         io.inversion.cloud.action.elastic.v03x.rql.Order order = orderList.get(i);
