...
public interface JSGet {
    ...
    default JSNode getNode(Object key) throws ClassCastException {
        return (JSNode) getJson().getValue(key);
    }

    default JSMap getMap(Object key) throws ClassCastException {
        return (JSMap) getJson().getValue(key);
    }

    /**
     * Convenience overloading of {@link JSNode#getValue(Object)}
     *
    ...
    default JSList getList(Object key) {
        return (JSList) getJson().getValue(key);
    }
    ...
    default String getString(Object key) {
        Object value = getJson().getValue(key);
        if (value != null)
    ...
    default int getInt(Object key) {
        Object found = getJson().getValue(key);
        if (found != null)
