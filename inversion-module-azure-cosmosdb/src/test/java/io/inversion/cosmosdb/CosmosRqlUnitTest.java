...
//                                                             "400 Bad Request - 400 Bad Request - The 'like' RQL operator for CosmosDb expects a single wildcard at the beginning OR the end of a value.  CosmosDb does not really support 'like' but compatible 'like' statements are turned into 'sw' or 'ew' statements that are supported."
        ...
        withExpectedResult("includes", "CosmosDb: SqlQuerySpec={\"query\":\"SELECT Northwind[\\\"shipCountry\\\"], Northwind[\\\"shipCity\\\"] FROM Northwind WHERE Northwind[\\\"type\\\"] = @type1 ORDER BY Northwind[\\\"id\\\"] ASC OFFSET 0 LIMIT 100\",\"parameters\":[{\"name\":\"@type1\",\"value\":\"orders\"}]} FeedOptions={enableCrossPartitionQuery=false}");

        withExpectedResult("distinct", "SqlQuerySpec={\"query\":\"SELECT DISTINCT Northwind[\\\"shipCountry\\\"] FROM Northwind WHERE Northwind[\\\"type\\\"] = @type1 ORDER BY Northwind[\\\"id\\\"] ASC OFFSET 0 LIMIT 100\",\"parameters\":[{\"name\":\"@type1\",\"value\":\"orders\"}]} FeedOptions={enableCrossPartitionQuery=false}");
        ...
        withExpectedResult("groupCount", "SqlQuerySpec={\"query\":\"SELECT Northwind[\\\"shipCountry\\\"], COUNT(*) AS \\\"countryCount\\\" FROM Northwind WHERE Northwind[\\\"type\\\"] = @type1 GROUP BY Northwind[\\\"shipCountry\\\"] ORDER BY Northwind[\\\"id\\\"] ASC OFFSET 0 LIMIT 100\",\"parameters\":[{\"name\":\"@type1\",\"value\":\"orders\"}]} FeedOptions={enableCrossPartitionQuery=false}");
