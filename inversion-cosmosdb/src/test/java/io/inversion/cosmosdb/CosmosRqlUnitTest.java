...
public class CosmosRqlUnitTest extends AbstractRqlTest {
    ...
    public CosmosRqlUnitTest() {
        ...
        withExpectedResult("distinct", "CosmosDb: SqlQuerySpec={\"query\":\"SELECT DISTINCT Northwind[\\\"shipCountry\\\"] FROM Northwind WHERE Northwind[\\\"type\\\"] = @type1 ORDER BY Northwind[\\\"id\\\"] ASC\",\"parameters\":[{\"name\":\"@type1\",\"value\":\"orders\"}]} FeedOptions={enableCrossPartitionQuery=false}");
        ...
        withExpectedResult("groupCount", "CosmosDb: SqlQuerySpec={\"query\":\"SELECT Northwind[\\\"shipCountry\\\"], COUNT(*) AS \\\"countryCount\\\" FROM Northwind WHERE Northwind[\\\"type\\\"] = @type1 GROUP BY Northwind[\\\"shipCountry\\\"] ORDER BY Northwind[\\\"id\\\"] ASC\",\"parameters\":[{\"name\":\"@type1\",\"value\":\"orders\"}]} FeedOptions={enableCrossPartitionQuery=false}");
