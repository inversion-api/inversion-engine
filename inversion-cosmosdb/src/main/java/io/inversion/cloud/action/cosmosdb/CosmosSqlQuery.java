...
public class CosmosSqlQuery extends SqlQuery<CosmosDb> {
    ...
    protected Where createWhere() {
        Where where = new Where(this) {
            protected Term transform(Term parent) {
                ...
                else if (!parent.isLeaf() && parent.hasToken("eq", "in", "has", "c") && collection.getPropertyByColumnName(parent.getToken(0)) != null && "array".equalsIgnoreCase(collection.getPropertyByColumnName(parent.getToken(0)).getType())) {
                    if (parent.getTerms().size() == 2) {
                        parent.withToken("c");
                    } //make an or to wrap each value
                    else {
                        List<Term> containsTerms = new ArrayList<>();
                        for (int i = 1; i < parent.getTerms().size(); i++) {
                            containsTerms.add(Term.term(null, "c", parent.getTerms().get(0), parent.getTerms().get(i)));
                        }
                        parent = Term.term(null, "or", containsTerms);
                    }

                }
                else if (!parent.isLeaf() && parent.hasToken("ne", "out", "nhas", "nc") && collection.getPropertyByColumnName(parent.getToken(0)) != null &&"array".equalsIgnoreCase(collection.getPropertyByColumnName(parent.getToken(0)).getType())) {
                    if (parent.getTerms().size() == 2) {
                        parent.withToken("nc");
                    } //make an or to wrap each value
                    else {
                        List<Term> containsTerms = new ArrayList<>();
                        for (int i = 1; i < parent.getTerms().size(); i++) {
                            containsTerms.add(Term.term(null, "nc", parent.getTerms().get(0), parent.getTerms().get(i)));
                        }
                        parent = Term.term(null, "and", containsTerms);
                    }
                }
        ...
        where.withFunctions("has", "nhas", "c", "nc");
        return where;
    }
    ...
    protected String printLimitClause(Parts parts, int offset, int limit) {
        ...
        if (limit <= 0)
            limit = 100;
        String s = "OFFSET " + offset + " LIMIT " + limit;
        parts.limit = s;
    ...
    protected String printExpression(Term term, List<String> dynamicSqlChildText, List<String> preparedStmtChildText) {
        ...
        //add other special cases here...@see SqlQuery.printExpression for examples
            ...
            if(preparedStmtChildText.size() != 2){
                ApiException.throw500InternalServerError("'c' must have only 2 child texts for printing. The column and the value it contains. This should be handled in the Where creation for multiple values. Found {}", preparedStmtChildText);
            }
            sql.append(format(" ARRAY_CONTAINS(%s, %s) ", preparedStmtChildText.get(0), preparedStmtChildText.get(1)));
        }
        else if ("nc".equalsIgnoreCase(token)) {
            if (preparedStmtChildText.size() != 2) {
                ApiException.throw500InternalServerError("'nc' must have only 2 child texts for printing. The column and the value it should not contains. This should be handled in the Where creation for multiple values. Found {}", preparedStmtChildText);
            }
            sql.append(format(" NOT ARRAY_CONTAINS(%s, %s) ", preparedStmtChildText.get(0), preparedStmtChildText.get(1)));
        }
        else {
